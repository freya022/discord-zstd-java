...
private static void printDeserializationTable(BenchmarkResults zlibBulkResults, BenchmarkResults zstdBulkResults, BenchmarkResults zlibStreamResults, BenchmarkResults zstdStreamResults) {
    ...
    System.out.println("| Metric averages | Bulk Zlib | Bulk Zstd | Streaming Zlib | Streaming Zstd |");
    System.out.println("|---------------------------|--------------------|-------------------|-------------------|-------------------|");
    System.out.printf("|Time per operation|%s|%s|%s|%s|\n", getScoreString(zlibBulkResults.primaryMetric), getScoreString(zstdBulkResults.primaryMetric), getScoreString(zlibStreamResults.primaryMetric), getScoreString(zstdStreamResults.primaryMetric));
    for (Map.Entry<String, BenchmarkResults.Metric> entry : zlibBulkResults.secondaryMetrics.entrySet()) {
        final String metricName = entry.getKey();
        final BenchmarkResults.Metric metric = entry.getValue();
        System.out.printf("|%s|%s|%s|%s|%s|\n", metricName, getScoreString(metric), getScoreString(zstdBulkResults, metricName), getScoreString(zlibStreamResults, metricName), getScoreString(zstdStreamResults, metricName));
    }
    System.out.println();
}
...
private static String getScoreString(BenchmarkResults results, String metricName) {
    var metric = results.secondaryMetrics.get(metricName);
    if (metric == null) return "N/A";

    if ("NaN".equals(metric.scoreError)) {
        return "%.3f %s".formatted(metric.score, metric.scoreUnit);
    } else {
        return "%.3f ± %.3f %s".formatted(metric.score, Double.valueOf(metric.scoreError), metric.scoreUnit);
    }
}

private static String getScoreString(BenchmarkResults.Metric metric) {
    if (metric == null) return "N/A";
